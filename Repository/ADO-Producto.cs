...
namespace WebApiCoder.Repository
{
    public class ADO_Producto
    {
        ...
        public static List<Producto> DevolverUsuario(int cod_usu)
        {
            var listaProductos = new List<Producto>();
            string connectionString = Connection.traerConnection();
            using (SqlConnection connection = new SqlConnection(connectionString))
            {
                connection.Open();
                SqlCommand cmd2 = connection.CreateCommand();
                cmd2.CommandText = "select pro.id,pro.Descripciones,pro.Costo,pro.PrecioVenta,pro.Stock,pro.idUsuario from Producto pro inner join Usuario usu on pro.IdUsuario = usu.Id where pro.IdUsuario = " + cod_usu.ToString();
                ...
                while (reader2.Read())
                {
                    ...
                    producto.Id = Convert.ToInt64(reader2.GetValue(0).ToString());
                    producto.Descripciones = reader2.GetValue(1).ToString();
                    producto.Costo = Convert.ToDouble(reader2.GetValue(2).ToString());
                    producto.PrecioVenta = Convert.ToDouble(reader2.GetValue(3).ToString());
                    producto.Stock = Convert.ToInt32(reader2.GetValue(4).ToString());
